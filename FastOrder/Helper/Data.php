...
class Data extends AbstractHelper
{
    ...
    public function getDataByListSkuQty($csvData, $storeId = 0)
    {
        ...
        if ($csvData && $sizeData > 1) {
            for ($i = 1; $i < $sizeData; $i++) {
                $item = $csvData[$i];
                $nameOrSku = trim($item[0]);
                $qty = isset($item[1]) ? (int)$item[1] : 1;
                $qty = $qty > 0 ? $qty : 1;

                if (!isset($listCsv[$nameOrSku]) || $qty > $listCsv[$nameOrSku]['qty']) {
                    $listCsv[$nameOrSku] = ['qty' => $qty];
                }
            }

            $lookupList = array_keys($listCsv);
        }
        ...
        $productList = $productCollection->addAttributeToSelect([
            "name", "sku", "price", "ptr", "discount", "attribute_set_id", "description", "has_options", "parent_id",
            "required_options", "short_description", "status", "store_id", "tax_class_id", "type_id",
            "url_key", "visibility", "msrp", "msrp_display_actual_price_type", "options_container",
            "quantity_and_stock_status", "image", "swatch_image", "thumbnail", "allow_stock"
        ])
        ->addAttributeToFilter([
            ['attribute' => 'name', 'in' => $lookupList],
            ['attribute' => 'sku', 'in' => $lookupList]
        ])
        ...
        foreach ($productList as $item) {
            $productName = trim($item->getName());
            $productSku = trim($item->getSku());

            $matchKey = null;
            if (isset($listCsv[$productSku])) {
                $matchKey = $productSku;
            } elseif (isset($listCsv[$productName])) {
            ...
            $product['fast_csv_qty'] = $listCsv[$matchKey]['qty'];
            $product['product_url'] = $item->getProductUrl();

            if (isset($product['price'])) {
                $productPrice = $product['price'];
                $newPrice = $this->priceCurrency->convert($productPrice);
                $product['base_price_value'] = $productPrice;
                $product['converted_new_price_value'] = $newPrice;
                $product['price'] = $this->priceCurrency->convertAndFormat($productPrice, false);
            }
            ...
            switch ($product['type_id']) {
                ...
                case self::$BUNDLE_TYPE_ID:
                    ...
                    foreach ($selectionCollection as $proSelection) {
                        $selectionArr[$proSelection->getOptionId()][] = [
                            'selection_id' => $proSelection->getSelectionId(),
                            'option_id' => $proSelection->getOptionId(),
                            'selection_product_name' => $proSelection->getName(),
                            'selection_base_price_value' => $this->priceCurrency->convert($proSelection->getPrice()),
                            'selection_product_price_format' => $this->priceCurrency->convertAndFormat($proSelection->getPrice(), false),
                            'selection_product_qty' => (int)$proSelection->getSelectionQty(),
                            'selection_product_id' => $proSelection->getProductId(),
                            'selection_is_default' => $proSelection->getIsDefault(),
                            'selection_price_type' => $proSelection->getSelectionPriceType(),
                            'selection_position' => $proSelection->getPosition()
                        ];
                    }
                    ...
                    foreach ($optionsCollection as $option) {
                        $optionLabel[$option->getOptionId()] = [
                            'option_id' => $option->getOptionId(),
                            'option_title' => $option->getDefaultTitle(),
                            'option_type' => $option->getType(),
                            'require' => $option->getRequired(),
                ...
                case self::$GROUPED_TYPE_ID:
                    ...
                    foreach ($associatedProducts as $productItem) {
                        ...
                        if (isset($productTemp['price'])) {
                            ...
                            $productTemp['base_price_value'] = $productTmpPrice;
                            $productTemp['converted_new_price_value'] = $newTmpPrice;
                            $productTemp['price'] = $this->priceCurrency->convertAndFormat($productTmpPrice, false);
                        }
            ...
            if (!empty($product['has_options']) && (int)$product['has_options'] == 1) {
                if (!empty($product['required_options']) && (int)$product['required_options'] == 1) {
                    $productOptions = $productObj->getOptions();
                    $config = [];
            ...
            // Keep only the product with the highest stock per match key
            if (!isset($allProductsByKey[$matchKey]) || $stock > $allProductsByKey[$matchKey]['stock']) {
                $allProductsByKey[$matchKey] = $product;
            }
        }
